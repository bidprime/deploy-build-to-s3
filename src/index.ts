...
class EventHandler {
  ...
  async handleEvent() {
    ...
    let deployedFiles = await this.unzipInputArtifactAndUploadFilesToWebsite(
      inputStream
    );
    ...
    if (this.userParameters.cleanAbsentFiles) {
      this.log(
        "Scrubbing old files, except for ignored list: ",
        this.userParameters.ignoreFiles
      );
      cleanupHadErrors = await this.removeAbsentFilesFromWebsite(
        deployedFiles,
        this.userParameters.ignoreFiles
      );
    }
  ...
  private async unzipInputArtifactAndUploadFilesToWebsite(
  ...
  private async removeAbsentFilesFromWebsite(
    deployedFiles: string[],
    ignoreFiles: string[]
  ): Promise<boolean> {
    let errors = await this.website.removeDifferences(
      deployedFiles.concat(ignoreFiles)
    );
    ...
    if (errors.length > 0) {
      ...
      this.failJob(
        "Failing job due to failure to cleanup excess files",
        errors[0]
      );
      return true;
    } else {
  ...
  private createWebsite(): Website {
    return new Website(
      this.userParameters.targetS3Bucket,
      this.userParameters.keyPrefix,
      (message, err) => {
